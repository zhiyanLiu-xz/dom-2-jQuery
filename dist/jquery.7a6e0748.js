...
window.$ = window.jQuery = function (selectorOrArrayOrTemplate) {
  ...
  if (typeof selectorOrArrayOrTemplate === "string") {
    elements = document.querySelectorAll(selectorOrArrayOrTemplate); //jQuery 返回的是一个对象，可以操作elements的api
    //api 可以操作elements
...
jQuery.fn = jQuery.prototype = {
  ...
  find: function find(selector) {
    ...
    var newApi = jQuery(array); //jQuery创建一个新的api

    return newApi; //不能直接return
  },
  ...
  children: function children() {
    ...
    this.each(function (node) {
      ...
      array.push.apply(array, _toConsumableArray(node.children)); //等价于 array.push(node.children[0],node.children[1],node.children[2],....)把里面的元素弄平
    });
    return jQuery(array);
  },
